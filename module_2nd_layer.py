...
def loss_function(x, y,pde,psy_trial,f):
    ...
    for xi in x:
        for yi in y:
            ...
            bet_out_w_all.append(net_out_w)
    ...
    net_out_all = np.array(net_out_all)
    bet_out_w_all = np.array(bet_out_w_all)
    net_out_jacobian_all = np.array(net_out_jacobian_all)
    net_out_hessian_all = np.array(net_out_hessian_all)
    psy_t_jacobian_all = np.array(psy_t_jacobian_all)
    psy_t_hessian_all  = np.array(psy_t_hessian_all)
    gradient_of_trial_d2x_all = np.array(gradient_of_trial_d2x_all)
    gradient_of_trial_d2y_all = np.array(gradient_of_trial_d2y_all)
    func_all = np.array(func_all)
    func_t_all = np.array(func_t_all)
    err_sqr_all = []
    psy_t_all = []
